...
//----------------------変更箇所------------------------------
                ...
                if (wall[hit_int_x, hit_int_y, hit_int_z] == 0)
                {
                    wall[hit_int_x, hit_int_y, hit_int_z] = 1;      
                }
    ...
    private IEnumerator DelayCoroutine()
    {
        ...
        int count = 1;
                ...
                while (true)
                {
                    ...
                    transform.position = Vector3.Lerp(transform.position,res[count],Time.deltaTime * 5.0f);
                    if(Vector3.Distance(transform.position,res[count])<0.1f)
                    {
                        count++;
                    }
                    Debug.Log(count);
                    if(Vector3.Distance(transform.position,goalInfo.cell)<0.1f)
                    {
                        break;
                    }
